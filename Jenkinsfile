        ...
        stage('Deploy and Service to Kubernetes') {
            steps {
                script {
                    ...
                    sh "kubectl apply -f ./yaml/demo-accounts.yaml -n ${NAMESPACE}"
                }
            }
        }
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/team5-eureka-client-account team5-eureka-client-account=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
